...
contract SMG is ERC20, Ownable {
    ...
    constructor() ERC20("SubMachine Gun", "SMG") {
        _mint(msg.sender, 0); // Mint initial supply (0 tokens) to the contract deployer
        storeNftAccessory(0, "Uzi", 50);
        storeNftAccessory(1, "Mac-10", 100);
        storeNftAccessory(2, "MP40", 200);
        storeNftAccessory(3, "FNP90", 250);
    }
    ...
    function redeem(uint256 accId) external returns (string memory) {
        ...
        require(balanceOf(msg.sender) >= redemptionAmount, "Balance must be more than or equal.");
